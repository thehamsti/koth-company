...
const DEFAULT_TWITCH_HTTP_TIMEOUT_MS = 10_000;
const MAX_TWITCH_HTTP_TIMEOUT_MS = 30_000;
...
export async function exchangeTwitchAuthorizationCode(
  ...
  const response = await twitchFetch(
    ...
    "Twitch authorization code exchange",
  );
  ...
  if (!response.ok || !data.access_token || !data.refresh_token) {
    throw new Error(`Failed to exchange Twitch code: ${data.message ?? response.statusText}`);
  }
  return {
...
function getTwitchHttpTimeoutMs(): number {
  const configured = process.env.TWITCH_HTTP_TIMEOUT_MS;
  if (configured === undefined) return DEFAULT_TWITCH_HTTP_TIMEOUT_MS;

  const timeoutMs = Number(configured);
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > MAX_TWITCH_HTTP_TIMEOUT_MS) {
    throw new Error(
      `TWITCH_HTTP_TIMEOUT_MS must be an integer between 1 and ${MAX_TWITCH_HTTP_TIMEOUT_MS}.`,
    );
  }
  return timeoutMs;
}
...
async function twitchFetch(
  ...
  const timeoutMs = getTwitchHttpTimeoutMs();
  ...
  try {
    ...
    if (signal.aborted || (error instanceof Error && error.name === "TimeoutError")) {
      throw new Error(
        `${operation} timed out after ${timeoutMs}ms. Confirm Twitch is reachable, then retry.`,
        { cause: error },
      );
    }
    throw error;
...
export async function refreshTwitchBroadcasterToken(refreshToken: string): Promise<TwitchTokens> {
  ...
  const response = await twitchFetch(
    ...
    "Twitch broadcaster token refresh",
  );
  const data = (await response.json()) as {
    access_token?: string;
    refresh_token?: string;
...
async function helix<T>(
  ...
  const response = await twitchFetch(
    ...
    `Twitch API request (${method} ${path})`,
  );
