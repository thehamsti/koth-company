...
async function loadState(): Promise<OperatorState> {
  const response = await fetch("/api/predictions/operator/commands", { cache: "no-store" });
...
export function OperatorControl({ initial }: { initial: OperatorState }) {
  ...
  async function command(value: Record<string, unknown>): Promise<boolean> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error?.message ?? "Command failed.");
      await refresh();
      return true;
    } catch (error) {
  ...
  async function configureTwitch(path: string, successMessage: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? "Twitch setup failed.");
  ...
  const heartbeatAt = state.automation?.lastHeartbeatAt
    ...
    : null;
  const automationStatus = !state.automation?.enabled
    ? "disabled"
    : state.automation.paused
      ? "paused"
      : !heartbeatAt || Date.now() - heartbeatAt > 15_000
        ? "stale"
        : "running";
  ...
  return (
    <main className="control-page">
                ...
                <section className="control-automation-panel" aria-labelledby="automation-title">
                  ...
                  <div className="automation-summary">
                    ...
                    <span>
                      ...
                      <b>
                        {heartbeatAt
                          ? `${Math.max(0, Math.round((Date.now() - heartbeatAt) / 1_000))}s ago`
                          : "Never"}
                      </b>
                    </span>
                  </div>
      ...
      <div className={`control-toast ${notice ? "is-visible" : ""}`} aria-live="polite">
        {notice}
      </div>
    </main>
  );
