...
export async function getPredictionSnapshot(userId?: string): Promise<PredictionSnapshot> {
  ...
  const [event] = await predictionDb
    .select()
    .from(events)
    .where(inArray(events.status, ["live", "draft"]))
  ...
  const portfolio = userId ? await getOrCreatePortfolio(event.id, userId) : null;
  ...
  const equityFor = (
    ...
    leaderboardPositions
      ...
      .reduce(
        (equity, position) =>
          equity.plus(
            new Decimal(position.shares).mul(priceByOutcome.get(position.outcomeId) ?? 0),
          ),
...
export async function createTradeQuote(input: {
  ...
  if (outcomeIndex < 0) throw new PredictionError("MARKET_NOT_FOUND", "Outcome not found.", 404);
  ...
  if (input.side === "buy" && new Decimal(portfolio.availableCrowns).lt(quote.crowns)) {
    throw new PredictionError("INSUFFICIENT_CROWNS", "Not enough Crowns for this position.");
  }
  if (input.side === "sell") {
    const [position] = await predictionDb
      .select()
      .from(positions)
      .where(and(eq(positions.portfolioId, portfolio.id), eq(positions.outcomeId, input.outcomeId)))
      .limit(1);
...
export async function executeTrade(input: {
  ...
  return predictionDb.transaction(async (tx) => {
    const [existing] = await tx
      ...
      .limit(1);
    if (existing) return { tradeId: existing.id };
